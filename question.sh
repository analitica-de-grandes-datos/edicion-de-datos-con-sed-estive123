...
# sed 's/\([0-9][0-9]\)\/\([0-9][0-9]\)\/\([0-9][0-9]\)/20\3-\2-\1/' data.csv | sed 's/\([0-9]\)\/\([0-9]\)\/\([0-9][0-9][0-9][0-9]\)/\3-\2-\1/' | sed 's/-\([0-9]\);/-0\1;/' | sed 's/-\([0-9]\)-/-0\1-/' | sed 's/;\([0-9][0-9][0-9]\),/;\1./' | sed -E 's/;/,/g' | sed 's/,\(\),/\,\\N\1,/' | sed 's/,\(\)*$/\,\\N\1/' | sed 's/a/A/' | sed 's/c/C/' | sed 's/n/N/' | sed 's/,\(N\)/\,\\N/'
